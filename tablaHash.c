...
/* Anadido por Manuel: para asegurarnos de que funciona strdup en Linux */
...
{
  unsigned long h = HASH_CODE_INI;
  unsigned char* p;

  for (p = (unsigned char*) str; *p; p++) {
      h = h*HASH_CODE_FACTOR + *p;
  }
  return h;
}
...
STATUS insertar_simbolo(TABLA_HASH* th, const char* lexema, CATEGORIA categ, TIPO tipo, CLASE clase, int adic1, int adic2)
{
  ...
  /* Insertar al principio de la lista de los nodos */
  n->next = th->tabla[ind];
  th->tabla[ind] = n;

  /* Insertamos al principio de la lista de simbolos */
  is->next = th->simbolos;
  th->simbolos = is;
...
int get_adicional1_FromSimbolo(INFO_SIMBOLO * is){
  return is ? is->adicional1 : -1;
}
int get_adicional2_FromSimbolo(INFO_SIMBOLO * is){
  return is ? is->adicional2 : -1;
}
...
if (categoria !=-1) sim->categoria = categoria;
if (tipo !=-1)  sim->tipo = tipo;
if (clase !=-1) sim->clase = clase;
if (adicional1 !=-1) sim->adicional1 = adicional1;
if (adicional2 !=-1) sim->adicional2 = adicional2;
return 0;
